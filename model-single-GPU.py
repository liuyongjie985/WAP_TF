...
class WAP():
    ...
    def get_sample(self, anno,
                   infer_y,
                   alpha_past,
                   h_pre,
                   if_trainning, p, w, h, alpha, ctx0, h_0, k, maxlen, stochastic, session, argmax, training):
...
def main(args):
    ...
    hidden_state_0 = tf.tanh(
        tf.tensordot(tf.reduce_mean(anno, axis=[1, 2]), wap.Wa2h, axes=1) + wap.ba2h)  # [batch, hidden_dim]
                ...
                if np.mod(uidx, sampleFreq) == 0:
                    ...
                    for batch_x, batch_y in valid:
                        for xx in batch_x:
                            ...
                            sample, score = wap.get_sample(anno,
                                                           infer_y,
                                                           alpha_past,
                                                           h_pre,
                                                           if_trainning, p, w, h, alpha, annot, h_state,
                                                           10, 100, False, sess, False, training=False)
